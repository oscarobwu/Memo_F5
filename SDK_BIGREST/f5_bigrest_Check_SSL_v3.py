...
#      Last modified: 2021-01-19 20:25
#     REVISION: ---
#===============================================================================
...
#    print(expiration)
    if subnnn is None:
        ...
        print("{}\t{}".format(v.properties.get('name'), ''.join(acc)))
#        if(((expiration.date() - datetime.date.today()).days <= 30 ) and ((expiration.date() - datetime.date.today()).days >= 0 )):
        if((expiration.date() - datetime.date.today()).days <= 30 ):
            tick = 1
#            print( '\t' + str((expiration.date() - datetime.date.today())) + '\t' + str(v.properties.get('name') + '\n' ))
            print( str(v.properties.get('name') + '\t' + str((expiration.date() - datetime.date.today())) + '\n' ))
#        print("{}".format(';'.join(acc)))
#        for x in n1:
#            acc.append(x.split(':')[1])
    #subname = v.properties.get('subjectAlternativeName').split(' ')
    #if not getattr(v, 'name', None):
        #print("{}\t{}\n".format(v.properties.get('name'), ''.join(acc)))
    #    print("{}, None".format(v.properties.get('name')))
    #    continue
